...
@[expose] public def parseInt (value : String) : Option (Int) := (LexLeanRuntime.parseDecimal (value) : Option (Int))

@[expose] public def parseInt8 (value : String) : Option (Int8) := (LexLeanRuntime.parseDecimal (value) : Option (Int8))

@[expose] public def parseInt16 (value : String) : Option (Int16) := (LexLeanRuntime.parseDecimal (value) : Option (Int16))

@[expose] public def parseInt32 (value : String) : Option (Int32) := (LexLeanRuntime.parseDecimal (value) : Option (Int32))

@[expose] public def parseInt64 (value : String) : Option (Int64) := (LexLeanRuntime.parseDecimal (value) : Option (Int64))

@[expose] public def parseUint8 (value : String) : Option (UInt8) := (LexLeanRuntime.parseDecimal (value) : Option (UInt8))

@[expose] public def parseUint16 (value : String) : Option (UInt16) := (LexLeanRuntime.parseDecimal (value) : Option (UInt16))

@[expose] public def parseUint32 (value : String) : Option (UInt32) := (LexLeanRuntime.parseDecimal (value) : Option (UInt32))

@[expose] public def parseUint64 (value : String) : Option (UInt64) := (LexLeanRuntime.parseDecimal (value) : Option (UInt64))

@[expose] public def acceptUInt8 (value : String) : Bool := (match (LexLeanRuntime.parseDecimal (value) : Option (UInt8)) with | Option.none => false | Option.some _ => true)

@[expose] public def nonzeroUInt16 (value : String) : Bool := (match (LexLeanRuntime.parseDecimal (value) : Option (UInt16)) with | Option.none => false | Option.some parsed => (if (LexLeanRuntime.equal (parsed) ((0 : UInt16)) : Bool) then false else true))

@[expose] public def boolByte (value : Bool) : ByteArray := (match value with | Bool.false => ByteArray.mk #[0] | Bool.true => ByteArray.mk #[1])

@[expose] public def entry (input : ByteArray) : ByteArray := (match (LexLeanRuntime.utf8Decode (input) : Option (String)) with | Option.none => ByteArray.mk #[255] | Option.some parsed => (match (LexLeanRuntime.parseDecimal (parsed) : Option (UInt8)) with | Option.none => (LexLeanRuntime.append (ByteArray.mk #[0]) (boolByte (nonzeroUInt16 (parsed))) : ByteArray) | Option.some _ => (LexLeanRuntime.append (ByteArray.mk #[1]) (boolByte (nonzeroUInt16 (parsed))) : ByteArray)))
